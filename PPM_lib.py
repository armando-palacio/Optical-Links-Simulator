...
def code(ook_s, M):
    """
    Recibe una secuencia de bits y la codifica en PPM. Codifica hasta 256-PPM.

    Entradas:
    ook_s: secuencia de bits a codificar
    ...
    ook_s = ook_s[:ook_s.size//k*k] # truncamos la secuencia de bits a un múltiplo de k

    n_simb = int(len(ook_s)/k)
    aux = np.zeros((n_simb,8), dtype=np.uint8); aux[:,-k:] = ook_s.reshape(n_simb,k) # dividimos la secuencia en símbolos de k bits
    decimal = np.packbits(aux, axis=1).reshape(n_simb) # convertimos los símbolos a decimal
    ppm_s = np.zeros(n_simb*M, dtype=int); ppm_s[decimal + np.arange(0,n_simb*M,M)] = 1 # codificamos los símbolos en PPM
    return ppm_s 

def decode(ppm_s, M, decision):
    """
    ...
    k = int(np.log2(M))
    n_simb = int(len(ppm_s)/M)

    symbols = np.reshape(ppm_s,(n_simb,M)) # convertimos la secueencia en una matriz de (n_simb x M)

    if decision == 'hard' or decision == 'HARD':
        i,j = np.where(symbols == 1) # filas y columnas de los 1's
        if len(set(i))!=n_simb: # si el número de filas no coincide con n_simb, hay símbolos en los que no se detectó ningún 1
            x = np.array(list(set(i)))
            x = np.insert( x, 0, range(n_simb, n_simb+np.sum(np.diff(x)-1)) )
            
            for i in set(range(n_simb))-set(x):
    ...
    ook_s = np.unpackbits(decimal, axis=1)[:,-k:].reshape(n_simb*k) # convertimos de decimal a binario y organizamos la matriz en una secuencia nuevamente
    return ook_s
...
def eye_params(ydata, sps, M):
    ...
    ydata = np.roll(ydata, -sps//2+1) # Para centrar el ojo en el gráfico
    ...
    umbral = 1/(s1**2-s0**2)*(mu0*s1**2-mu1*s0**2+s1*s0*((mu1-mu0)**2+2*(s1**2-s0**2)*np.log(s1/s0*(M-1)))**0.5)
    ...
    return EYE(t,ydata,t_left,t_right,t_dist,t_center,mu0,s0,mu1,s1,umbral,instant,y_bot,y_top,er,eye_h,eye_w,jitter,ampx,sps,M)
...
def BER_TEO_H(M,R,nf,g,fo,Ps,Dv,Df,T):
    """
    Esta función calcula la BER teórica para el caso de un receptor con decodificación dura.
    ...
    pe_slot_min = np.min(1/M*Qfunc((mu1-x)/s1) + (M-1)/M*Qfunc((x-mu0)/s0)) 
    pe_symb = 1-(1-pe_slot_min)**M
    return M/2/(M-1)*pe_symb

def BER_TEO_S(M,R,nf,g,fo,Ps,Dv,Df,T):
    """
    Esta función calcula la BER teórica para el caso de un receptor con decodificación suave.
    ...
    mu0,S0 = mu0_S0(R,nf,g,fo,Dv,Df,T); s0 = S0**0.5
    mu1,S1 = mu1_S1(R,nf,g,fo,Ps,Dv,Df,T); s1 = S1**0.5
    pe_symb = 1-1/(2*pi)**0.5*integ.quad(lambda x: (1-Qfunc((mu1-mu0+s1*x)/s0))**(M-1)*np.exp(-x**2/2),-np.inf,np.inf)[0]
    return M/2/(M-1)*pe_symb

#### Definimos las clases necesarias ####
class Parametros:
    def __init__(self, P_laser, bit_rate, nbits, sps, BW_opt, BW_elec, G_rx, NF_rx, G_tx, landa, Responsivity, orden_elec, orden_opt, CHANNEL_LOSS, M):
        self.Ptx = P_laser
        self.ptx = idbm(P_laser)
        self.M = M
        self.k = int(np.log2(M))
        self.bit_rate = bit_rate
        self.slot_rate = bit_rate*M/self.k
        self.nbits = (nbits//self.k)*self.k
        self.nslots = self.nbits//self.k*M
        self.sps = sps
        self.BW_opt = BW_opt
        ...
        self.g_tx = idb(G_tx)
        self.landa = landa
        self.fo = self.c/(landa)
        self.R = Responsivity
        self.i_dark = 10e-9
        self.nt = self.nslots*sps
        self.dt = 1/self.slot_rate/sps
        self.T_windows = self.nslots / self.slot_rate
...
class EYE:
    def __init__(self, t, y, t_left, t_right, t_bit, t_opt, mu0, std0, mu1, std1, umbral, instant, cluster0, cluster1, er, eye_h, eye_w, jitter, ampx, sps, M):
        self.x = t
        self.y = y
        self.t_left = t_left
    ...
    def __str__(self):
        ...
        return '\n'.join([mu0, std0, mu1, std1, umbral, t_opt, i, eye_h, eye_w, jitter, ampx])
...
class Transmisor:
    def __init__(self, parameters, prbs_type = 'random', user_prbs = None):
        ...
        self.ppm_s = self.PPM_ENCODER(self.ook_s)

        # dominio eléctrico
        self.nrz_pulses = self.NRZ_PULSE_SHAPE(self.ppm_s)
    ...
    def PPM_ENCODER(self, seq):
        return code(seq, self.p.M)

    def NRZ_PULSE_SHAPE(self, seq, type = 'rect'):  
        x = np.kron(seq, np.ones(self.p.sps))
        if type == 'rect':
            return x
...
class Receptor:
    def __init__(self, parameters, signal_in, prbs_tx):
        self.p = parameters
        self.IN = signal_in
        self.prbs_tx = prbs_tx

        tic()
        self.BER_teo_H, self.BER_teo_S, self.I0_teo, self.I1_teo, self.sigma0_teo, self.sigma1_teo, self.umbral_teo = self.TEO_PARAMETERS()
        ...
        self.OSNR = db(self.p.M*self.p_sig_mean/self.p_ase)
        ...
        # ADC (HARD DECISION)
        self.out_ppm_seq = self.ADC(self.y_PD_filt, bits=1)

        # ADC (SOFT DECISION)
        self.out_ppm_sig = self.ADC(self.y_PD_filt, bits=16)

        # decodifico la secuencia ppm recibida (Hard Decision Decoding)
        self.out_H = self.PPM_DECODER(self.out_ppm_seq, decision = 'hard')

        # decodifico la señal analógica recibida (Soft Decision Decoding)
        self.out_S = self.PPM_DECODER(self.out_ppm_sig, decision = 'soft')

        self.BER_count_H = self.BER_COUNTER(self.prbs_tx, self.out_H)
        self.BER_count_S = self.BER_COUNTER(self.prbs_tx, self.out_S)

        self.BER_est_H, self.BER_est_S, self.I0_est, self.I1_est, self.sigma0_est, self.sigma1_est = self.SIM_PARAMETERS()
        toc()
    ...
    def EYE_PROCESS(self, ydata):
        return eye_params(ydata, self.p.sps, self.p.M)

    def DOWN_SAMPLING(self, signal_over_sampled, instant):
        return signal_over_sampled[instant::self.p.sps]

    def ADC(self, signal, bits):  
        signal_down = self.DOWN_SAMPLING(signal, self.eye.i)
        
        if bits == 1:
            return signal_down > self.eye.umbral
        elif bits > 1:
            # levels = np.linspace(0, 2*self.eye.mu1, 2**bits)
            # return find_nearest(levels, signal_down)
            return signal_down
          
    def PPM_DECODER(self, ppm_s, decision):
        return decode(ppm_s, self.p.M, decision)
    ...
    def TEO_PARAMETERS(self):
        M = self.p.M
        P1 = self.p.g_rx * M * power(self.IN)  # Potencia pico del símbolo '1'
        ...
        umbral = 1/(s1**2-s0**2)*(I0*s1**2-I1*s0**2+s1*s0*((I1-I0)**2+2*(s1**2-s0**2)*np.log(s1/s0*(M-1)))**0.5)
        Pe_slot = 1/M*Qfunc((I1-umbral)/s1) + (M-1)/M*Qfunc((umbral-I0)/s0)
        Pe_symb_H = 1-(1-Pe_slot)**M
        BER_H = M/2/(M-1)*Pe_symb_H

        pe_symb_S = 1-1/(2*pi)**0.5*integ.quad(lambda x: (1-Qfunc((I1-I0+s1*x)/s0))**(M-1)*np.exp(-x**2/2),-np.inf,np.inf)[0]
        BER_S = M/2/(M-1)*pe_symb_S

        return BER_H, BER_S, I0, I1, s0, s1, umbral
    ...
    def PRINT_TEO_PARAMS(self):
        ...
        print('   BER_H = %.2e' % self.BER_teo_H)
        print('   BER_S = %.2e' % self.BER_teo_S)
        print()
    ...
    def SIM_PARAMETERS(self):
        ...
        return BER_H, BER_S, I0, I1, s0, s1
    ...
    def PRINT_SIM_PARAMS(self):
        ...
        print('   BER_H = %.2e' % self.BER_est_H)
        print('   BER_S = %.2e' % self.BER_est_S)
        print()
    ...
    def EYE_DIAGRAM(self, nbits=1000, title='', show_=True, save=False):
        ...
        eye = self.eye
        ax[0].plot(eye.x[self.p.sps: nbits*self.p.sps], eye.y[self.p.sps: nbits*self.p.sps], 'k.', lw=0.2, alpha=0.15)
   
        suptitle('Diagrama de ojo, BER_est_Hard = %.1e' % (self.BER_est_H))
        ...
        plot(1/self.p.M*gaus(x, eye.mu1, eye.s1), x, color = '#E18F00')
        plot((self.p.M-1)/self.p.M*gaus(x, eye.mu0, eye.s0), x, color = '#E18F00')
    ...
    def SIG_BEFORE_THEN_FILT(self, nslots = 15):
        sec_plot = np.arange(self.p.nt/2, self.p.nt/2 + nslots*self.p.sps, dtype=int)
        plot(self.p.t[sec_plot]*self.p.slot_rate, self.y_PD[sec_plot]*1e3, 'b', alpha=0.5, label='señal detectada')
        plot(self.p.t[sec_plot]*self.p.slot_rate, self.y_PD_filt[sec_plot]*1e3, 'red', label = 'señal filtrada')
        plot(self.DOWN_SAMPLING(self.p.t, self.eye.i)*self.p.slot_rate, self.DOWN_SAMPLING(self.y_PD_filt, self.eye.i)*1e3, 'ko', label = 'decisión')
        axhline(self.eye.umbral*1e3, color = 'k', ls='--', label = 'umbral estimado')
        axhline(self.umbral_teo*1e3, color = 'y', ls='--', label = 'umbral teórico')
        xticks(np.arange(0,nslots+1))
        grid()
        xlabel('Time [t/Tb]')
        ylabel('Amplitud [u.a.]')
        xlim(0,nslots)
